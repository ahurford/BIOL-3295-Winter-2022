...
The answer to this question, in general, is 'no' because the different stage classes increase by different amounts, however, for special values of the population sizes in each stage class at time, $t$, the equivalence relationship between $\lambda$ and $\mathbb{P}$ does hold. These special values are called the _stable stage structure_ (or _stable age structure_ if the population is grouped by age). When the stable age structure is reached, all stages increase by the same factor, $\lambda$, each time step, such that:
